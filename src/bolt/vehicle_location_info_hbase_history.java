...
public class vehicle_location_info_hbase_history extends BaseRichBolt {
	...
	public void prepare(Map config, TopologyContext context, OutputCollector collector) {
		...
		cfg.set("hbase.rootdir", "hdfs://hdfs1.hhdata.com:8020/apps/hbase/data");//使用eclipse时必须添加这个，否则无法定位
	...
	public void execute(Tuple tuple) {
		...
		try{
			if(line.contains("{")&&line.contains("}")&&line.contains("strCompanyId")){//合法的json文件进行解析
				if(rowNum%5000==0)
